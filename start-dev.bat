@echo off
REM ─────────────────────────────────────────────────────
REM  PyBackup Sentinel — Development Launcher (Windows)
...
start "PyBackup-Backend" cmd /k "cd /d %~dp0backend && venv\Scripts\python -m uvicorn backend.main:app --reload --host 0.0.0.0 --port 8000 --app-dir .."
...
start "PyBackup-Frontend" cmd /k "cd /d %~dp0frontend && npm run dev"
